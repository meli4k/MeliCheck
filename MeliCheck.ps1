...
$outputFile = Join - Path - Path $desktopPath - ChildPath
"PcCheckLogs.txt"
if (Test - Path $outputFile)
{
    Remove - Item - Path $outputFile - Force
Write - Host
"Old PcCheckLogs.txt file deleted." - ForegroundColor
Yellow
}
}
...
$envOneDrive =[System.IO.Path]::
    Combine($env: UserProfile, "OneDrive")
...
{
Write - Error
"Unable to find OneDrive path: $_"
return $null
}
}
...
@("C:\Users\$userName\Documents\My Games\Rainbow Six - Siege", "$oneDrivePath\Documents\My Games\Rainbow Six - Siege")
...
{ $rarSearchPaths += $_.Root}
$oneDrivePath = Get - OneDrivePath
if ($oneDrivePath)
{ $rarSearchPaths += $oneDrivePath}
...
{
$allFiles += $_.FullName
if ($_.FullName -like "*OneDrive*")
{ $oneDriveFiles += $_.FullName}
}
}
return $allFiles
}
...
$oneDriveFiles | Sort - Object | ForEach - Object
...
{
    Write - Output
"Finding suspicious files names..."
$desktopPath = [System.Environment]::GetFolderPath('Desktop')
$outputFile = Join - Path - Path $desktopPath - ChildPath
"PcCheckLogs.txt"
...
Write - Output
"Log file not found. Unable to search for suspicious files."
}
}
...
{
    Write - Host
"Logging reg entries inside PowerShell..." - ForegroundColor
DarkYellow
$desktopPath = [System.Environment]::GetFolderPath('Desktop')
$outputFile = Join - Path - Path $desktopPath - ChildPath
"PcCheckLogs.txt"
...
Write - Host
" Fetching UserSettings Entries " - ForegroundColor
Blue
$registryPath = "HKLM:\SYSTEM\CurrentControlSet\Services\bam\State\UserSettings"
$userSettings = Get - ChildItem - Path $registryPath | Where - Object
{ $_.Name - like
"*1001"}
...
Write - Host
"No relevant user settings found." - ForegroundColor
Red
}
Write - Host
"Fetching Compatibility Assistant Entries"
...
Write - Host
"Fetching AppsSwitched Entries" - ForegroundColor
Blue
...
Write - Host
"Fetching MuiCache Entries" - ForegroundColor
Blue
...
Get - Content $outputFile | Sort - Object | Get - Unique | Where - Object
{ $_ - notmatch
"\{.*\}"} | ForEach - Object
{ $_ - replace
...
Write - Host
"Opening stats for $name on Stats.cc ..." - ForegroundColor
Blue
Start - Process $url
Start - Sleep - Seconds
0.5
...
Write - Host
" Fetching Downloaded Browsers " - ForegroundColor
Blue
function
Log - BrowserFolders
{
...
Write - Host
"Registry path for browsers not found." - ForegroundColor
Red
}
}
...
{
    Write - Host
"Logging Windows install date..." - ForegroundColor
DarkYellow
...
"`n-----------------"
Add - Content - Path $outputFile - Value
"`nWindows Installation Date: $installDate"
}
...
$outputFile = Join - Path - Path([System.Environment]::GetFolderPath('Desktop')) -ChildPath
"PcCheckLogs.txt"
$prefetchHeader = "`n-----------------`nPrefetch Files:`n"
if (Test - Path $prefetchFolderPath)
{
$prefetchFiles = Get - ChildItem - Path $prefetchFolderPath - Filter
"*.pf" | ForEach - Object
{
...
$desktopPath = [System.Environment]::GetFolderPath('Desktop')
$logFilePath = Join - Path - Path $desktopPath - ChildPath
"PcCheckLogs.txt"
...
Write - Host
"Log file copied to clipboard." - ForegroundColor
DarkRed
} else {
    Write - Host
"Log file not found on the desktop." - ForegroundColor
Red
}
$desktopPath = [System.Environment]::GetFolderPath('Desktop')
...
$downloadsPath = Join - Path - Path $userProfile - ChildPath
"Downloads"
...
Remove - Item - Path $filePath - Force - ErrorAction
SilentlyContinue
}
}

$targetFileDesktop = Join - Path - Path $desktopPath - ChildPath
"PcCheck.txt"
$targetFileDownloads = Join - Path - Path $downloadsPath - ChildPath
"PcCheck.txt"
